...
func (s *SmartContract) CreateAsset(ctx contractapi.TransactionContextInterface, assetID, publicDescription string) error {
	...
	transientMap, err := ctx.GetStub().GetTransient() // Transient data is private to the application-smart contract interaction. It is not recorded on the ledger and is often used in conjunction with private data collections
	if err != nil {
		return fmt.Errorf("error getting transient: %v", err)
	}

	// Asset properties must be retrieved from the transient field as they are private
	immutablePropertiesJSON, key := transientMap["asset_properties"]
	if !key {
	...
	assetBytes, err := json.Marshal(assetCreate)
	if err != nil {
		return fmt.Errorf("failed to create asset JSON: %v", err)
	}

	err = ctx.GetStub().PutState(assetCreate.ID, assetBytes)
	if err != nil {
	...
	if err != nil {return fmt.Errorf("failed setting state based endorsement for owner: %v", err)}*/

	// Persist private immutable asset properties to owner's private data collection
	collection := buildCollectionName(clientOrgID)
	err = ctx.GetStub().PutPrivateData(collection, assetCreate.ID, immutablePropertiesJSON)
...
func (s *SmartContract) UpdateAsset(ctx contractapi.TransactionContextInterface, assetID string, newDescription string) error {
	...
	assetUpdate, err := s.ReadAsset(ctx, assetID)
	if err != nil {
	...
	assetUpdate.PublicDescription = newDescription
	updatedAssetJSON, err := json.Marshal(assetUpdate)
	if err != nil {
...
func (s *SmartContract) TransferAsset(ctx contractapi.TransactionContextInterface, assetID string, buyerOrgID string) error {
	...
	immutablePropertiesJSON, key := transMap["asset_properties"]
	if !key {
		return fmt.Errorf("asset_properties key not found in the transient map")
	}

	priceJSON, key := transMap["asset_price"]
	if !key {
